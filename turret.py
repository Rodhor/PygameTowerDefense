...
class Turret(pg.sprite.Sprite):
    def __init__(self, sprite_sheet, tile_x, tile_y):
        ...
        ### Update image ###
        self.angle = 90
        self.original_image = self.animate_list[self.frame_index]
        self.image = pg.transform.rotate(self.original_image, self.angle)
        self.rect = self.image.get_rect()
        self.rect.center = (self.x, self.y)

        # create transparent cirkle to show range
        self.range_image = pg.Surface((self.range * 2, self.range * 2))
        self.range_image.fill((0, 0, 0))
        self.range_image.set_colorkey((0, 0, 0))
        pg.draw.circle(
            self.range_image, "gray100", (self.range, self.range), self.range
        )
    ...
    def update(self, enemy_group):
        # If a target is in range, play shooting animation
        if self.target:
            self.play_animation()
        else:
    ...
    def pick_target(self, enemy_group):
        ...
        # Check distance form each enemy if they are in range
